...
test('DashboardTokenList 会渲染全部 token 并使用默认布局样式', () => {
  ...
  assert.match(html, /rounded-full border border-primary\/25 bg-primary\/10 px-3 py-1\.5 text-sm text-primary/)
})
...
test('DashboardTokenList 在没有 token 时会渲染空状态文案', () => {
  ...
  assert.match(html, /rounded-full border border-border bg-card px-3 py-1.5 text-sm text-muted-foreground/)
})
