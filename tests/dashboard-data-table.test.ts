...
test('DashboardDataTable 会渲染默认表格容器、表头与 tbody 内容', () => {
  ...
  assert.match(
    html,
    /dashboard-scroll-area overflow-x-auto overflow-y-hidden touch-pan-x rounded-lg border border-border bg-card shadow-card cursor-grab/u,
  )
  assert.match(html, /w-full min-w-max divide-y divide-surface-200/)
  assert.match(html, /bg-muted\/50/)
  assert.match(html, /divide-y divide-surface-200 bg-card/)
  assert.match(
    html,
    /whitespace-nowrap px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider/u,
  )
