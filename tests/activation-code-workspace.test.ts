...
test('ActivationCodeWorkspace 在 results tab 渲染绑定设备列、管理入口与单码管理面板', () => {
  const html = renderToStaticMarkup(
    React.createElement(
  ...
  assert.equal(html.includes('管理'), true)
  assert.equal(html.includes('单码管理'), true)
  assert.equal(html.includes('自助换绑：允许（来源：项目配置）'), true)
  assert.equal(html.includes('次数上限：2 次（来源：项目配置）'), true)
...
test('ActivationCodeWorkspace 在 results tab loading 或空数据时渲染加载与空状态', () => {
  ...
  assert.equal(emptyHtml.includes('选择一条激活码后，可在此查看绑定设备、最终生效策略并执行强制解绑或换绑。'), true)
})
