...
test('DashboardSummaryStrip 会渲染默认容器样式与两侧内容', () => {
  ...
  assert.match(html, /mb-5 rounded-lg border border-border bg-muted\/50 px-5 py-4/)
...
test('DashboardSummaryStrip 支持覆盖外层与内层样式', () => {
  ...
  assert.doesNotMatch(html, /mb-5 rounded-lg border border-border bg-muted\/50 px-5 py-4/)
})
