...
test('buildSystemConfigPageModel 会按访问控制、认证安全和系统展示分组', () => {
  ...
  assert.deepEqual(
    ...
    ['allowedIPs', 'allowAutoRebind', 'autoRebindCooldownMinutes', 'autoRebindMaxCount'],
  )
  assert.deepEqual(
...
test('buildSystemConfigPageModel 会补充用于设置页展示的预览与推荐元数据', () => {
  ...
  const autoRebindSwitchItem = model.groups[0].items.find((item) => item.key === 'allowAutoRebind')
  const autoRebindCooldownItem = model.groups[0].items.find(
    (item) => item.key === 'autoRebindCooldownMinutes',
  )
  const autoRebindMaxCountItem = model.groups[0].items.find(
  ...
  assert.deepEqual(autoRebindSwitchItem?.badges, [{ label: '允许自动换绑', tone: 'success' }])
