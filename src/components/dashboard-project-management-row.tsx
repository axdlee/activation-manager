  ...
  return (
    <tr className="transition hover:bg-surface-50">
      <td className="px-6 py-4 text-sm text-ink-900">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isDefaultProject ? (
              <span className="inline-flex items-center rounded-full border border-brand-100 bg-brand-50 px-2.5 py-1 text-[11px] font-medium text-brand-700">
                默认项目
              </span>
            ) : null}
          </div>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-ink-500">
        <div className="space-y-2">
          <div className="font-mono text-sm text-ink-700">{project.projectKey}</div>
      ...
      <td className="px-6 py-4 text-sm text-ink-500">
        <div className="max-w-sm space-y-2">
          {policySummary.map((item) => (
            <div
              key={item}
              className="rounded-md border border-surface-200 bg-surface-50 px-4 py-3 text-sm leading-6 text-ink-600"
            >
              {item}
            </div>
          ))}
