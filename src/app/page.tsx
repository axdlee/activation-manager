    ...
    <main className={publicPageClassName}>
      <div className={publicContainerClassName}>
        ...
        <section className={`${publicShellClassName} animate-fade-in-up overflow-hidden`}>
          <div className="flex flex-col gap-10 p-6 sm:p-8 lg:flex-row lg:items-start lg:p-12">
            <div className="max-w-2xl flex-1">
              ...
              <h1 className="mt-5 text-3xl font-semibold tracking-tight text-ink-50 sm:text-4xl lg:text-5xl">
                {t('home.title', '激活码管理系统')}
              </h1>
              <p className="mt-4 max-w-xl text-base leading-7 text-ink-500">
                {t('home.subtitle', '面向多项目、多授权模型与插件接入场景的一体化后台。发码、查码、看消费趋势，并把公开 API 文档直接交给接入方。')}
              </p>

              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/admin/login" className={publicPrimaryButtonClassName}>
                  {t('home.enterAdmin', '进入管理后台')}
                </Link>
                <Link href="/shop" className={publicPrimaryButtonClassName}>
                  {t('home.buyLicense', '购买激活码')}
                </Link>
                <Link href="/docs/api" className={publicSecondaryButtonClassName}>
                  {t('home.viewApi', '查看 API 文档')}
                </Link>
              </div>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className={`${publicShellClassName} p-6 sm:p-8`}>
            <h2 className="text-lg font-semibold tracking-tight text-ink-50">典型使用场景</h2>
            <ul className="mt-6 space-y-5">
              {[
                { title: '按次扣减', text: '浏览器插件按真实调用次数扣减 remainingCount，见底即失效。' },
                { title: '周期授权', text: '桌面工具首次激活后按有效期持续授权，到期自动失效。' },
                { title: '多项目并行', text: '同一服务端同时管理多个产品或客户项目，互不干扰。' },
                { title: '链路可查', text: '通过消费日志、趋势图与 requestId 快速回查问题。' },
              ].map((item, index) => (
          ...
          <div className={`${publicShellClassName} p-6 sm:p-8`}>
            ...
            <div className="mt-6 space-y-4">
              {[
                { title: t('nav.admin', '管理后台'), description: t('home.adminDesc', '项目管理、发码、消费日志与系统配置工作台。'), href: '/admin/login', primary: true },
                { title: t('nav.shop', '购买激活码'), description: t('home.shopDesc', '选择套餐、在线下单，支付成功后自动发放卡密。'), href: '/shop', primary: false },
                { title: t('nav.apiDocs', 'API 文档'), description: t('home.apiDesc', '正式接口、字段规范、SDK 示例与联调方法。'), href: '/docs/api', primary: false },
              ].map((item) => (
                <div key={item.title} className="flex flex-col gap-4 rounded-lg border border-surface-200 bg-surface-50 p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                  ...
                  <Link href={item.href} className={`shrink-0 ${item.primary ? publicPrimaryButtonClassName : publicSecondaryButtonClassName}`}>
                    立即进入
                  </Link>
                </div>
              ))}
