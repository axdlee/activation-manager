...
| P4-04 | P4 | DONE | 收口 README / 开发文档 / 安全说明 | 保证文档与实现一致 |
| P4-05 | P4 | DONE | 增加关键操作审计日志 | 提升可追踪性与合规性 |
...
- page.tsx 由 3488 行降至 3033 行（-455 行），全部 hook 均保持行为等价（解构别名 + 页面适配层）；新增 11 个纯函数测试
...
### 2026-09-02 / Iteration 2026-09i：P4 收口（审计补齐 + 文档一致性）+ 安全头收紧

**目标**：收口 P4-04（文档一致性）与 P4-05（关键操作审计），补齐审计覆盖缺口。

**已完成**：

- [x] P4-05 补齐三条关键操作审计：
  - 激活码删除 `CODE_DELETED`（删除前写审计，避免 FK 约束；记录激活码串与项目）
  - 清理过期激活码 `CODE_CLEANUP_EXPIRED`（记录清理数量与码 id 列表）
  - 管理员登录成功 `ADMIN_LOGIN`（记录来源 IP）
- [x] 新增审计类型同步到 `AdminOperationType` 与审计中心筛选下拉
- [x] 3 个新测试覆盖新审计路径（tests/admin-audit-additions.test.ts）
- [x] P4-04：README 安全章节同步（CSP 生产收紧无 `unsafe-eval`、安全响应头、CSV 防注入、审计操作清单、e2e 门禁说明）
- [x] next.config.js CSP 按环境区分：生产去掉 `'unsafe-eval'`，dev 保留（HMR 需要）

**验证结果**：

1. `npm run lint` ✅
2. `npm test` ✅（353 / 353，新增 3 个审计测试）
3. `npm run test:coverage` ✅（92.48 / 85.99 / 90.18）

**备注**：

- 删除激活码前先写审计：删除后 Prisma 默认将审计行 `activationCodeId` 置空（SetNull），审计行仍保留激活码串与项目归属
- 清理是批量操作，审计不绑定单个激活码，按操作类型 + 管理员聚合可查
